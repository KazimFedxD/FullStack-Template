...
Architecture:
- Uses Django's cache backend (configured in settings.CACHES)
- Backend: django-redis (RedisCache) for production-grade caching
- Shared with django-ratelimit for consistent Redis usage
- All cache operations go through Redis (redis://redis:6379/1)

Usage:
    from utils.cache import app_cache, session_cache, page_cache

    # Set value
    app_cache.set('key', 'value', timeout=300)

    # Get value
    value = app_cache.get('key', default='default_value')
    ...
    def clear(self) -> bool:
        ...
        try:
            if self.prefix:
                ...
                logger.warning(
                    "Clearing prefixed cache - this operation may be expensive"
                )
            self._backend.clear()
            return True
        except Exception as e:
